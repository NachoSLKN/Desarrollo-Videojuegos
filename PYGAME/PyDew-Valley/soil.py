...
class WaterTile(pygame.sprite.Sprite): #Clase idéntica a la class Generic pero por mantener código limpio y ordenado
    def __init__(self, pos, surf, groups):
        super().__init__(groups)
        self.image = surf
        self.rect = self.image.get_rect(topleft = pos)
...
class SoilLayer:
    ...
    def create_hit_rects(self):
        ...
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'F' in cell: #Creamos un rectángulo para cada celda que tiene una F de farmeable, podemos ver el mapa en tile y en el array mostrado por consola (40x50) donde la mayor parte farmeable es mitad del mapa hacia abajo.
    ...
    def water(self, target_pos): # Calculamos si este target position choca con cualquiera de los soil sprites
             for soil_sprite in self.soil_sprites:
                  if soil_sprite.rect.collidepoint(target_pos):
                    ...
                    # 1. Añadir entrada a la grilla de soil -> 'W'
    ...
    def water_all(self):
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'X' in cell and 'W' not in cell: #Reviamos toda la grilla y revisamos si hay una X y si no tiene agua aún
                    cell.append('W')
                    x = index_col * TITLE_SIZE
                    y = index_row * TITLE_SIZE
                    WaterTile(
    ...
    def plant_seed(self,target_pos, seed):
        for soil_sprite in self.soil_sprites.sprites():
            if soil_sprite.rect.collidepoint(target_pos):
                ...
                self.plant_sound.play() #Solo se reproducirá cuando golpeemos el harado
